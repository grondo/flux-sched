...
PENDING_TIMEOUT=${PENDING_TIMEOUT:-5}
...
# Number of nodes the scheduler believes are free, as opposed to the
# number the resource module believes are free. The difference is the
...
state_is_clean () {
	...
	test $(sched_free_nnodes) -eq $SIZE
}
...
'
test_expect_success 'scheduler sees all nodes free' '
	test $(sched_free_nnodes) -eq $SIZE
'
...
for fmt in $MATCH_FORMATS; do
	...
	test_expect_success "$fmt/$scenario: hold resources" '
		...
		test $(sched_free_nnodes) -eq $(expected_free)
	'
